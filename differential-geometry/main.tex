...
\begin{nthm}
    Let $\alpha = \alpha(s)$ be a unit speed curve in with $\kappa > 0$. This is a plane curve if and only if $\tau = 0$ for all $s$.
\end{nthm}
\begin{proof}
    If $\tau = 0$, then the Frenet-Serret equations reduce to
    \begin{equation}
        \dot{\vT} = \kappa \vN,
        \quad
        \dot{\vN} = -\kappa \vT,
...
\begin{nthm}
    ...
    \begin{equation}
        A = \frac{1}{2} \int_a^b \paren*{x\dot{y} + y\dot{x}} \,dt
    \end{equation}
\end{nthm}
\begin{proof}
    Exercise. Use the Green's theorem to show that this is equal to $A(\gamma)$ from before.
\end{proof}
